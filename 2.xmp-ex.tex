...
Moreover, the {\tt from\_device} clause specifies the device which has broadcast variables.
...
If there is no {\tt from\_device} clause, it is assumed that the default device set is specified by {\tt on\_device} clause.
...
In line 6,
the {\tt bcast} directive broadcasts the variable {\it a} stored on accelerator memory on d(1) of p(2) to all accelerator memories of all {\it nodes}.

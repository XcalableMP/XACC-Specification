...
\begin{itemize}
\item A {\bf device set} is a set of accelerator devices on a {\bf node}. {\OACC} directives can be applied to a {\bf device set}.
\item A {\bf distributed-array} is an array distributed on a {\bf device set}. You can offload and distribute arrays on host to {\bf device set}.
Moreover, the extension directives can offload works and copy memory between host and {\bf device set} for the {\bf distributed-arrays}.
\item The extension directives can synchronize devices in a {\bf device set}.
\item {\XACC} directives also transfer data between device memories on the {\bf node}.
\end{itemize}
